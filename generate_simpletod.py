...
def main():
    ...
    if args.test_file is None:
        ...
        if args.output_path is not None:
            json.dump({"input":prompt_text,"output":texts},open(args.output_path,"w"),indent=4)
        ...
        output = []
        progress_bar = tqdm(total = math.ceil(len(test_dataset)/args.per_device_test_batch_size))
        model.eval()
        for i in range(0,len(test_dataset),args.per_device_test_batch_size):
            batch = [x['text'][-1024:] if len(x['text'])>1024 else x['text'] for x in test_dataset[i:i+args.per_device_test_batch_size]]
            encoded_prompt = tokenizer(batch, padding="longest", return_tensors="pt")
            encoded_prompt = encoded_prompt.to(args.device)
            ...
            for generated_sequence in output_sequences:
                ...
                # Remove all text after the stop token
                text = text[: text.find(args.stop_token) if args.stop_token else None].replace("<|pad|>","").strip()
