...
def main():
    ...
    if args.test_file is None:
        ...
        for dialogue in test_dataset:
            history = {}
            for batch in dialogue['dialogue']:


                history_list = [k+" "+v for k, v in history.items()]
                history_belief = " <|history|> " + ", ".join(history_list) + " <|endofhistory|>" if history_list else "<|history|> <|endofhistory|>"
                encoded_prompt = tokenizer.encode(batch['text'] + history_belief, add_special_tokens=False, return_tensors="pt")
                encoded_prompt = encoded_prompt.to(args.device)
                ...
                if text.find("<|belief|>") != -1 and text.find("<|endofbelief|>") != -1 and text.count("<|belief|>") == 1 and text.count("<|endofbelief|>") == 1:
                    state = text[text.find('<|belief|>'):text.find('<|endofbelief|>')].replace('<|belief|>',"").strip()
                else:
                    state = ""
                state = state.split(",")
                state = [s.strip().split() for s in state]
                for s in state:
                    if len(s) >= 3:
                        history[s[0]+" "+s[1]] = (" ".join(s[2:])).strip()
